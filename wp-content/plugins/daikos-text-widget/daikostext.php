...
function widget_daikos_text_init()
{
	...
	function widget_daikos_text_control($number) {
      ...
      $newoptions[$number]['href'] = strip_tags(stripslashes($_POST["daikos-text-href-$number"]));
    ...
    $href = htmlspecialchars($options[$number]['href'], ENT_QUOTES);
	...
	?>
        ...
        <label for="daikos-text-href-<?php echo "$number"; ?>" title="Title href above the widget" style="line-height:35px;display:block;">Href:<input style="width: 470px;" id="daikos-text-href-<?php echo "$number"; ?>" name="daikos-text-href-<?php echo "$number"; ?>" type="text" value="<?php echo $href; ?>" /></label>
 ...
 /* Do the conditional tag checks. */
		switch ($show) {
			case "all": 
				echo $before_widget;
				echo '<div class="DaikosText ' . (strtolower(str_replace(' ', '-', $title))) . '">';
        if (empty($href)) {
          $title ? print($before_title . $title . $after_title) : null;
        } else {
          $title ? print($before_title . '<a href="' . $href . '">' . $title . '</a>' . $after_title) : null;;
        }
			  eval('?>'.$text);
				echo "</div>"; 
				echo $after_widget."

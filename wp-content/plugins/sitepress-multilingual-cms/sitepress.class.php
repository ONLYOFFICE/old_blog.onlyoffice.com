	...
	function rel_canonical()
	{
		...
		if ( $master_post_id = get_post_meta( $id, '_icl_lang_duplicate_of', true ) ) {
			...
			if(preg_match('/\?lang=/', $link)){
				$languser = $this->get_language_tag( $code );
				$masslanguser = preg_split('/-/', $languser);
				$alternativelink = preg_replace('/\?lang=[a-z]{2}/', "" ,$link);
				$alternativelink = preg_replace('/\/blog/', "/".$masslanguser[0]."/blog" , $alternativelink);
				echo "<link rel='canonicaltest' href='$alternativelink' />\n";
			} else {
				echo "<link rel='canonicaltest' href='$link' />\n";
			}
	...
	function head_langs()
	{
		...
		if ( $is_valid ) {
			foreach ( $languages as $code => $lang ) {
				...
				if(preg_match('/\?lang=/', $lang[ 'url' ])){
					$languser = $this->get_language_tag( $code );
					$masslanguser = preg_split('/-/', $languser);
					$alternativelink = preg_replace('/\?lang=[a-z]{2}/', "" ,$lang[ 'url' ]);
					$alternativelink = preg_replace('/\/blog/', "/".$masslanguser[0]."/blog" , $alternativelink);

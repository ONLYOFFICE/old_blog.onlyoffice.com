...
Version: 1.9.7
Plugin URI: https://rick.jinlabs.com/code/twitter
Description: Displays your public Twitter messages for all to read. Based on <a href="https://cavemonkey50.com/code/pownce/">Pownce for Wordpress</a> by <a href="https://cavemonkey50.com/">Cavemonkey50</a>.
Author: Ricardo Gonz&aacute;lez
Author URI: https://rick.jinlabs.com/
*/
	...
	if ($username == '') {
        ...
        $isFirst = true;
				foreach ( $messages->items as $message ) {
					...
					if ($list) echo '<li class="twitter-item' . ($isFirst ? ' first' : '') . '">'; elseif ($num != 1) echo '<p class="twitter-message">';
...
function hyperlinks($text) {
    ...
    //$text = preg_replace("/\b(www\.[a-z][a-z0-9\_\.\-]*[a-z]{2,6}[a-zA-Z0-9\/\*\-\?\&\%]*)\b/i","<a href=\"https://$1\" class=\"twitter-link\">$1</a>", $text);
    $text = preg_replace('/\b(?<!:\/\/)(www\.[\w_.\-]+\.[a-zA-Z]{2,6}[\/\w\-~.?=&%#+$*!]*)\b/i',"<a href=\"https://$1\" class=\"twitter-link\">$1</a>", $text);    
    
    // match name@address
    $text = preg_replace("/\b([a-zA-Z][a-zA-Z0-9\_\.\-]*[a-zA-Z]*\@[a-zA-Z][a-zA-Z0-9\_\.\-]*[a-zA-Z]{2,6})\b/i","<a href=\"mailto://$1\" class=\"twitter-link\">$1</a>", $text);
        //mach #trendingtopics. Props to Michael Voigt
    $text = preg_replace('/([\.|\,|\:|\�|\�|\>|\{|\(]?)#{1}(\w*)([\.|\,|\:|\!|\?|\>|\}|\)]?)\s/i', "$1<a href=\"https://twitter.com/#search?q=$2\" class=\"twitter-link\">#$2</a>$3 ", $text);
    return $text;
}
...
function widget_twitter_init() {
  ...
  if ($check_options['number']=='') {
	...
	function widget_twitter($args, $number = 1) {
		...
		$messages = fetch_rss('https://api.twitter.com/1/statuses/user_timeline/'.$item['username'].'.rss');


		// These lines generate our output.
    echo $before_widget . $before_title . '<a href="https://twitter.com/' . $item['username'] . '" class="twitter_title_link">'. $item['title'] . '</a>' . $after_title;
